...
function StatisticsPage() {
  ...
  const weeklyTasks = loadFromStorage(`weeklyTasks_${userKey}`, {});
  const weekData   = weekDates.map(({ label, fullDate }) => {
    const dayTasks = weeklyTasks[fullDate] || [];
  ...
  const habitsByMonth = loadFromStorage(`habitsByMonth_${userKey}`, {});
  const track         = loadFromStorage(`track_${userKey}`, {});
  const habits        = habitsByMonth[monthKey] || [];
  const days          = generateMonthDays(today.getFullYear(), today.getMonth());
  const habitData     = habits.map(habit => {
    let completed = 0;
    for (let d = 1; d <= days.length; d++) {
      if (track[`${habit}-${monthKey}-${d}`]) {completed++;}
    }
    const pct = days.length > 0 ? Math.round((completed / days.length) * 100) : 0;
    return { name: habit.length > 20 ? habit.slice(0, 20) + "…" : habit, pct };
  });
  ...
  // ── Monthly Budget ────────────────────────────────────────────────────────
  const expenses        = loadFromStorage(`expenses_${userKey}`, []);
  ...
  // ── Project Planner ───────────────────────────────────────────────────────
  const projects = loadFromStorage(`projects_${userKey}`, []);
    ...
    <div className={`${common.container} ${common.pageEnter}`}>
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.chartBox}>
          ...
          {totalTasksAll === 0
            ? <p className={styles.empty}>No tasks added yet this week.</p>
            : (
        ...
        <div className={styles.chartBox}>
          ...
          {habitData.length === 0
            ? <p className={styles.empty}>No habits tracked this month.</p>
            : (
              <ResponsiveContainer width="100%" height={220}>
        ...
        <div className={styles.chartBox}>
          ...
          {budgetData.length === 0
            ? <p className={styles.empty}>No expenses recorded this month.</p>
            : (
              <ResponsiveContainer width="100%" height={220}>
        ...
        <div className={styles.chartBox}>
          ...
          {totalProjectTasks === 0
            ? <p className={styles.empty}>No project tasks added yet.</p>
            : (
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie
